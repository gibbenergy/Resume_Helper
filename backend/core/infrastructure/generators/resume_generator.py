...
from backend.core.utils.file_utils import atomic_write_json, atomic_read_json
...
class ResumeGenerator:
    ...
    def format_profile_data(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        clean = {
            'name_prefix': profile.get('name_prefix', ''),
    ...
    def export_json(self, profile_data: Dict[str, Any], output_path: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            data['_export_timestamp'] = str(uuid.uuid1())
            
            write_result = atomic_write_json(data, output_path)
            
            if write_result.is_success():
                logger.info(f"[Request {request_id}] JSON file successfully exported")
                return True, ""
            else:
                error_msg = write_result.get_error_message()
    ...
    def generate_pdf(self, profile: Dict[str, Any], output_path: str, template_style: str = None) -> bool:
        """Generate PDF resume from profile data using Pyppeteer."""
        try:
            ...
            if os.path.exists(output_path):
                ...
                logger.info(f"PDF successfully generated at: {output_path} (Size: {file_size} bytes)")
                ...
                logger.error(f"PDF generation failed: File not created at {output_path}")
                return False
