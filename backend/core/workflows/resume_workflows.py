...
class ResumeAIWorkflows:
    ...
    def tailor_resume(self, resume_data: Dict, job_description: str, model: Optional[str] = None, user_prompt: Optional[str] = None, job_analysis_data: Optional[Dict[str, Any]] = None) -> Dict:
        ...
        4. **CRITICAL: Optimize the summary field** - The input resume has a "professional_summary" field containing a "summary" key. 
           You MUST tailor this summary to align with the position requirements using existing qualifications.
           Return the optimized summary in the same structure (either as "summary" at top level, or within "professional_summary").
        ...
        OUTPUT REQUIREMENTS:
        - Return same JSON structure with optimized wording of EXISTING content only
        - **MUST include an optimized/tailored summary field** that matches the job requirements
        - The summary should be rephrased to emphasize relevant qualifications from the original resume
        - You can return summary either as a top-level "summary" field OR within "professional_summary" structure
        """
        ...
        if result["success"]:
            try:
                ...
                # Log AI response structure for debugging
                logger.info(f"🔍 AI Response Structure: keys = {list(tailored_professional_content.keys())}")
                if 'professional_summary' in tailored_professional_content:
                    ps = tailored_professional_content['professional_summary']
                    logger.debug(f"🔍 AI professional_summary type: {type(ps).__name__}, is_dict: {isinstance(ps, dict)}")
                    if isinstance(ps, dict):
                        logger.debug(f"🔍 AI professional_summary keys: {list(ps.keys())}")
                if 'summary' in tailored_professional_content:
                    logger.debug(f"🔍 AI has top-level summary (length: {len(str(tailored_professional_content['summary']))})")
                ...
                normalized_content = {}
                logger.debug(f"🔍 Starting normalization, sanitized_resume keys: {list(sanitized_resume.keys())}")
                ...
                # Check for summary in sanitized_resume - it's in professional_summary after sanitization
                if 'professional_summary' in sanitized_resume or 'summary' in sanitized_resume:
                    logger.debug(f"🔍 Summary extraction: sanitized_resume keys = {list(sanitized_resume.keys())}")
                    logger.debug(f"🔍 Summary extraction: professional_summary exists = {'professional_summary' in sanitized_resume}")
                    
                    ai_summary = None
                    
                    # Try multiple extraction strategies to handle different AI response formats
                    # Strategy 1: Check top-level 'summary' field
                    if 'summary' in tailored_professional_content:
                        ai_summary = tailored_professional_content['summary']
                        logger.debug(f"✅ Found summary at top level in AI response")
                    
                    # Strategy 2: Check 'professional_summary' as dict with 'summary' key
                    if ai_summary is None and 'professional_summary' in tailored_professional_content:
                        professional_summary = tailored_professional_content['professional_summary']
                        if isinstance(professional_summary, dict) and 'summary' in professional_summary:
                            ai_summary = professional_summary['summary']
                            logger.debug(f"✅ Found summary in professional_summary dict")
                        elif isinstance(professional_summary, str):
                            # professional_summary is a string directly
                            ai_summary = professional_summary
                            logger.debug(f"✅ Found summary as professional_summary string")
                    
                    # Validate and use the extracted summary
                    if ai_summary and isinstance(ai_summary, str) and ai_summary.strip():
                        normalized_content['summary'] = ai_summary.strip()
                        logger.info(f"✅ Successfully extracted AI-tailored summary (length: {len(ai_summary)})")
                    else:
                        # Get original summary from correct location (professional_summary.summary after sanitization)
                        if 'professional_summary' in sanitized_resume:
                            prof_summary = sanitized_resume['professional_summary']
                            original_summary = prof_summary.get('summary', '') if isinstance(prof_summary, dict) else ''
                        else:
                            original_summary = sanitized_resume.get('summary', '')
                        
                        normalized_content['summary'] = original_summary
                        if original_summary:
                            logger.warning(f"⚠️ AI output missing or invalid 'summary' (got type: {type(ai_summary).__name__}), using original summary (length: {len(original_summary)})")
                        else:
                            logger.warning(f"⚠️ AI output missing or invalid 'summary', and no original summary found")
                ...
                logger.debug(f"🔍 Normalized content keys: {list(normalized_content.keys())}")
                if 'summary' in normalized_content:
                    logger.info(f"🔍 Normalized content has summary (length: {len(normalized_content['summary'])})")
                else:
                    logger.warning(f"🔍 Normalized content does NOT have summary field")
                
                logger.debug(f"🔍 About to merge: normalized_content has summary = {'summary' in normalized_content}")
                complete_tailored_resume = PrivacyManager.merge_personal_info_for_documents(
                    normalized_content, personal_info
                )
                logger.debug(f"🔍 After merge: complete_tailored_resume keys = {list(complete_tailored_resume.keys())}")
                if 'personal_info' in complete_tailored_resume and 'summary' in complete_tailored_resume.get('personal_info', {}):
                    logger.info(f"🔍 Final resume has personal_info.summary (length: {len(complete_tailored_resume['personal_info']['summary'])})")
                elif 'summary' in complete_tailored_resume:
                    logger.info(f"🔍 Final resume has top-level summary (length: {len(complete_tailored_resume['summary'])})")
                else:
                    logger.warning(f"🔍 Final resume does NOT have summary in expected locations")
